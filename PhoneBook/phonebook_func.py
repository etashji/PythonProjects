...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_phonebook (col_fname, col_lname, col_fullname, col_phone, col_email) VALUES (?,?,?,?,?)""", (data, data, data, data, data)
            conn.close()
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_fname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # enforce the user to provide both
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
...
def onDelete(self):
    ...
    conn = seqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
...
######              onReferesh(self) # update the listbox of the changes
                conn.commit()
            else:
...
def onDeleted(self):
    ...
    self.txt_phone.delete(0,END)
    self.txt_email.delete(0,END)
##      onRefresh(self) # update the lixtbox of the changes
    try:
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            print(count2)
            if count == 0 or count2 == o: # if proposed changes are not already in the database, then proceed
                response = messagebox.askokcancel("Update Request", "The following changes ({}) and ({}) will be implemented for ({}). \n..."
                print(response)
                if response:
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_phonebook SET col_phone = '{0}', col_email = '{1}' WHERE col_fullname = \n\nYour up""")
                        onClear(self)
                        conn.commit()
                else:
